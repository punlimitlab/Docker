    ...
    process {
        ...
        if ($removeConflictingContainers -and $hostPort) {
            ...
            if ($conflictingContainerIds) {
                ...
                'Stopping conflicting containers {0}' -f $conflictingContainerIds | Write-Verbose
                $command = 'docker{0} -H {1} stop {2}' -f $authOptions, $dockerServerUrl, $conflictingContainerIds
